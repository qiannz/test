...
function clearCache() {
	$('#clear_cache').bind('click', function() {
		$(this).html('更新中...').unbind('click');
		var url = '/admin/index/clear-cache/t:' + new Date().getTime();
		$.getJSON(url, {}, function(data){
			$('#clear_cache').html('更新缓存');
			alert(data.msg);
			clearCache();
		});
    })
...
function addHistoryItem(tab, item){
    ...
    if($(id).length == 1){
        ...
        var close = $('<a href="javascript:;" class="close"><img src="/css/admin/images/close.gif" / ></a>');
        lnk.click(function(){
            openItem(item, tab);
        });
...
function setWorkspace(){
	...
	if(wWidth > 1400) {
		...
		$('#workspace').width(wWidth - $('#left').width() - parseInt($('#left').css('margin-right')) - 18);
	}
