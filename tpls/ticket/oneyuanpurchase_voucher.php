...
<!--					<p class="address-bottom"><img src="/images/c.png" class="c-img"><img src="/images/a.png" class="a-img"></p>-->
			...
			<div class="buy-wrap">
                ...
                {{if $ticketRow.voucher_status eq 1}}
                    {{if $ticketRow.is_notice eq 1}}
                    <input type="button" class="buy-btn" value="已提醒" />
                    {{else}}
                    <input type="button" class="buy-btn" id="buy-btn-prompt" data-ticketid="{{$ticketRow.ticket_id}}"  value="提醒我" />
                    {{/if}}                
                {{else}}
				<input type="button" class="buy-btn" id="buy-btn" value="{{if $ticketRow.ticket_mark=='spike'}}立即购买{{else}}立即参与{{/if}}" />
                {{/if}}
			</div>
		</div>
    </div>
