<form action="{{$_CONF.FORM_ACTION}}/ctype/{{$coupon_type}}/sid/{{$sid}}" method="post" id="couponForm" enctype="multipart/form-data">
...
<div class="upFileBox">
    ...
    <p class="error">
    <span class="h2" id="p_value_error"></span>
    <span class="h2" id="total_error"></span>
    </p>
    
    <p class="inputBox">
        <label class="label">领取有效期：</label>
    ...
    <p class="submitBox" ><input type="button"  class="submit" value="提交优惠券" onClick="submit_check()" id="submitCheck"></p>
</div>
</form>
...
var validArray = ['t_title', 'p_value', 'total', 'sdate', 'edate', 'stime', 'etime', 'summary', 'file_img', 'postTextarea'];
...
function submit_check() {
	...
	if(len == validArray.length) {
		$("form#couponForm").submit();
		$("form#couponForm .submit").attr('value', '提交中。。。').attr('disabled', true);		
	}
	return false;
}
