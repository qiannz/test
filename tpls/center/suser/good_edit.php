...
$(function(){
	...
	uploadSwf = new SWFUpload({
		...
		post_params: {"user_name" : "{{$user.user_name}}", "primary_id" : "{{$goodInfo.good_id}}"},
            ...
            <div class="upFileBox">
                    ...
                    <div class="imgBox" id="postImageList" {{if !$goodInfo.good_id}}style="display:none"{{/if}}>
                        ...
                        {{foreach from=$imgList key=key item=item}}
                            <li>
                                <p class="{{if $item.is_first eq 1}}sel{{else}}picBox{{/if}}"><img src="{{$_CONF.SITE_URL}}/data/good/small/{{$item.img_url}}"></p>
                                <p class="setBox">
                                <a class="del" data-aid="{{$item.good_img_id}}">删除</a>
                                <a class="setCover" data-aid="{{$item.good_img_id}}" data-gid="{{$goodInfo.good_id}}">设为封面</a>
