...
<body>
    <div class="viewport" style="background:#3b3663">
        <div class="login-wrap">
                <div class="login-top-pic">
                    <img src="/images/commodity/login-top-pic.jpg" />
                </div>
                <!---登录-->
                <div class="login-box">
                    <div class="login-col">
                            <input type="text" name="mobile" id="mobile" maxlength="11" class="input_50"  placeholder="输入手机号码"/>
                            <input type="button" class="yzm-on" value="获取验证码" id="getCode"/>
                     </div>
                    <p class="login-error" id="mobile_error"></p>
                    <div class="login-col">
                        <input type="text" name="code" id="code" maxlength="4" class="input_100"  placeholder="输入短信验证码"/>
                    </div>
                    <p class="login-error" id="code_error"></p>

                    <input type="button" id="login" class="login-btn"  value="登 录"/>
                </div>
...
<script type="text/javascript">
    ...
    function sendCode(mobile){
		if(!checkMobile(mobile)){
            ...
            $.ajax({
                type:'POST',
                url:'/home/message/send-code',
                data:{mobile:mobile},
                dataType:'json',
                success:function(data){
                    if(data.returncode == 100) {
                    	fnEvent('getCode');
                        $("#mobile_error").html('验证发送成功');
                    } else{
                        $("#mobile_error").html('验证发送失败');
                    }
                }
            });
        }
    ...
    function verifyMobileCode(mobile,code){	
		if(!checkMobile(mobile)){
            ...
            $.ajax({
                type:'POST',
                url:'/home/message/veriy-mobile',
                data:{mobile:mobile, code:code},
                dataType:'json',
                success:function(data){
                    if(data.returncode == 100){
						$.cookie('API_CLIENT_USER_ID', data.result.user_id);  
						$.cookie('API_CLIENT_SHOP_ID', data.result.shop_id);
						window.location.href = '/home/message/thread';				
                    } else {
						$("#code_error").html(data.returnmessage);
					}
                }
            })
        }
