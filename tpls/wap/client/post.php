...
<body>
    <div class="viewport">
        <!--header-->
        <div class="header"><a class="go-back" href="/home/message/thread"></a>消息<!--a class="del"></a--></div>
        <!--对话-->
        <div class="talk-wrap">
            ...
            <div class="news-col">
                ...
                <div class="news-img-box">
                    <a class="news-img" style="background-image: url({{$data.good.first_img.img_url}})"></a>
                    <img src="/images/commodity/news-blank.png">
                </div>
                <!--右-->
                <div class="news-right">
                    <a class="shop-title">{{$data.good.ticket_title}}</a>
                    <span class="shop-price">{{$data.good.selling_price}}</span>
                </div>
            </div>
            <!--talk list--->
            <div class="talk-list">

				{{foreach from=$data.detail item=item}}
                <div class="talk-col">
                		{{if $item.position eq 'L'}}
                        <div class="talk-info">
                            <p class="left">
                                    <span class="head-pic" style="background-image: url({{$item.avator}})"></span>
                                    <span class="name">{{$item.user_name}}</span>
                             </p>
                            <p class="right color">{{$item.format_time}}</p>
                            <div class="msg-box right blue-bg">
                                {{$item.question}}
                            </div>
                        </div>
                        {{else}}
                        <div class="talk-col">
                            <div class="talk-info">
                                <p class="right">
                                    <span class="name{{if $item.is_my_message eq 1}} color-pink{{/if}}">{{$item.user_name}}</span>
                                    <span class="head-pic" style="background-image: url({{$item.avator}})"></span>
                                </p>
                                <p class="left color">{{$item.format_time}}</p>
                                <div class="msg-box left gray-bg">
                                    {{$item.question}}
                                </div>
                            </div>
                        </div>                       
            ...
            <!--p class="look-more"><a id="link-more">查看更多</a></p>
			<script>
                var icount = 1;
    ...
    <div class="popup-wrap">
        ...
        <div class="input-wrap">
            ...
            <div class="text-box">
                 <textarea name="qst" id="qst"></textarea>
            </div>
        ...
        <input type="hidden" id="frid" value="{{$frid}}" />
    </div>
...
$(function(){
	$(".send-btn").click(function(){
		...
		$.ajax({
			type:'POST',
			url:'/home/message/info-reply',
			data:{tid:tid, uid:uid, sid:sid, frid:frid, qst:qst},
			dataType:'json',
			success:function(data){
				if(data.returncode == 100) {
					alert('留言回复成功！');
					window.location.href = '/home/message/post/tid/' + tid + '/frid/' + frid;
				}
			}
		});		
