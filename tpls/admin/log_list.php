...
<div class="mrightTop">
  <div class="fontl">
    <form method="post" action="{{$_CONF.FORM_ACTION}}">
       <div class="left">
            一级模块：
			<select name="pmodule" id="pmodule">{{$smarty.request.pmodule}}###{{$smarty.request.cmodule}}
            	<option value="">所有</option>
            	{{foreach from=$pmodule key=key item=item}}
            	<option value="{{$item.mark}}" {{if $smarty.request.pmodule eq $item.mark}} selected="selected"{{/if}}>{{$item.m_name}}</option>
            	{{/foreach}}
        	</select>&nbsp;&nbsp;&nbsp;
            二级模块：
			<select name="cmodule" id="cmodule">
            	...
            	<option value="{{$item.mark}}" {{if $smarty.request.cmodule eq $item.mark}} selected="selected"{{/if}}>{{$item.m_name}}</option>
            	{{/foreach}}
        	</select> &nbsp;&nbsp;&nbsp;
        	操作动作：
