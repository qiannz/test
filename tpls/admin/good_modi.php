...
$(function(){
	...
	uploadSwf = new SWFUpload({
		...
		post_params: {"user_name" : "{{$row.user_name}}", "primary_id" : "{{$row.good_id}}"},
...
<div id="rightTop">
  ...
  <ul class="subnav">
    ...
    <li><span>{{if $row.good_id}}编辑{{else}}新建{{/if}}商品</span></li>
  </ul>
</div>
<div class="info">
...
<table class="infoTable">
        ...
        <td class="paddingT15 wordSpacing5">
            ...
            <div class="imgBox" id="postImageList" {{if !$row.good_id}}style="display:none"{{/if}}>
            	...
            	{{foreach from=$imgList key=key item=item}}
                	<li>
                        <p class="img"><a href="{{$_CONF.SITE_URL}}/data/good/original/{{$item.img_url}}" target="_blank"><img src="{{$_CONF.SITE_URL}}/data/good/small/{{$item.img_url}}"></a></p>
    ...
    <tr>
        ...
        <td class="ptb20">
            <a href="javascript:checkSubmit()" id="sub"><img src="/images/good.png"/></a>
        </td>
    </tr>
</table>
</form>
