...
<div class="tdare" style="min-width:1400px;">
  ...
  <table width="100%" cellspacing="0" class="dataTable">
    ...
    <tr class="tatr2">
      ...
      <td>
      		<a href="/admin/spike/add-edit/tid:{{$item.ticket_id}}/sid:{{$item.shop_id}}/uname:{{$item.user_name}}/page:{{$page}}">编辑</a> | 
            {{if $item.ticket_status eq '1'}}
            <a href="/admin/spike/recommend/id:{{$item.ticket_id}}/type:{{$item.ticket_type}}/page:{{$page}}">推荐</a> | 
            {{/if}}
            {{if $item.ticket_status eq 0}}
            <a href="/admin/spike/audit/tid:{{$item.ticket_id}}/sid:{{$item.shop_id}}/page:{{$page}}">审核</a> |
            {{/if}}
