...
function checkSubmit()
{
	if($("#form").valid())
	{
		...
		if ($("#content").val(editor.html()).length == 0){
			$('#ts').html('此用户不是商家认证用户，不能添加代金券');	
		}
...
<div id="rightTop">
  <p>券管理</p>
  <ul class="subnav">
    <li><a class="btn1" href="/admin/ticket/voucher-list">代金券列表</a></li>
    {{if !$row.ticket_id}}<li><a class="btn3" href="/admin/ticket/user-shop/type:v/uname:{{$uname}}">店铺选择列表</a></li>{{/if}}
    <li><span>{{if $row.ticket_id}}编辑代金券{{else}}新建代金券{{/if}}</span></li>
  </ul>
</div>
<div class="info">
...
<table class="infoTable">
    ...
    <tr>
        ...
        <td class="paddingT15 wordSpacing5">
          <input class="infoTableInput2" type="text" name="par_value" id="par_value" value="{{$row.par_value|string_format:"%d"}}" />
          <label class="field_notice"></label>
        </td>
    </tr>
    <tr>
        <th class="paddingT15">优惠券售价:</th>
        <td class="paddingT15 wordSpacing5">
          <input class="infoTableInput2" type="text" name="selling_price" id="selling_price" value="{{$row.selling_price|string_format:"%d"}}" />
          <label class="field_notice"></label>
        </td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td class="paddingT15 wordSpacing5">
          ...
          <select name="limit_unit" id="limit_unit">
                <option value="场" {{if $row.limit_unit eq '场'}} selected="selected" {{/if}}>场</option>
                <option value="小时" {{if $row.limit_unit eq '小时'}} selected="selected" {{/if}} >小时</option>
                <option value="天" {{if $row.limit_unit eq '天'}} selected="selected" {{/if}} >天</option>
                <option value="周" {{if $row.limit_unit eq '周'}} selected="selected" {{/if}} >周</option>
                <option value="月" {{if $row.limit_unit eq '月'}} selected="selected" {{/if}}>月</option>
            </select>  
          <label class="field_notice"></label>
        </td>
    </tr>
    ...
    <tr>
        ...
        <td class="paddingT15 wordSpacing5">
          ...
          <label class="field_notice" id="ts"></label>
        </td>
    </tr>
    <tr>
