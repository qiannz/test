 ...
 *
 */
class Controller_Admin_Crowdfunding extends Controller_Admin_Abstract {
	...
	public function __construct() {
		parent::__construct();
		$this->_model = Model_Admin_Crowdfunding::getInstance();
		$this->_img_width = 750;
		$this->_img_height = 350;
		$this->_buy_discount = array(
					'1' => '逢双减半'
				);
	}
	...
	public function listAction() {
		...
		$this->_tpl->display('admin/crowdfunding_list.php');
	}
	/**
	 * 众筹新增与编辑
	 */
	public function addEditAction() {
		...
		if($tid) {
			$row = $this->_model->getCrowdfundingRow($tid);
		}
		if($this->_http->isPost()) {
			...
			if($getData['tid']) {
				$resultArr = Model_Admin_Ticket::getInstance()->addEditCrowdfundingTicket($getData, $userInfo, $this->_ad_city);
				if($resultArr['status'] == 100) {
					Custom_Log::log($this->_userInfo['id'], "编辑了一元众筹： <b>{$getData['ticket_title']}</b> ID：{$getData['tid']}", $this->pmodule, $this->cmodule, 'edit', 'ticket', $getData['tid']);
					//WAP图片上传
					...
					Custom_Common::showMsg('一元众筹编辑失败');
				}
			} else {
				$resultArr = Model_Admin_Ticket::getInstance()->addEditCrowdfundingTicket($getData, $userInfo, $this->_ad_city);
				if($resultArr['status'] == 100) {
					Custom_Log::log($this->_userInfo['id'], "新增了一元众筹： <b>{$getData['ticket_title']}</b> ID：{$resultArr['insert_ticket_id']}", $this->pmodule, $this->cmodule, 'add', 'ticket', $resultArr['insert_ticket_id']);
					//WAP图片上传
					...
					Custom_Common::showMsg(
						...
						'',
						array(
							'add-edit/sid:'. $getData['sid'] . '/uname:' . $getData['user_name'] . '/page:' . $getData['page']   => '继续新增一元众筹',
							'list/page:' . $getData['page'] => '返回一元众筹'
						)
					);
				} else {
					Custom_Common::showMsg('一元众筹新增失败');
				}
			}
		}
		...
		$this->_tpl->assign('buyDiscount', $this->_buy_discount);
		$this->_tpl->display('admin/add_crowdfunding.php');
	}
	...
	public function userShopAction() {
		...
		$this->_tpl->display('admin/shop_choose_crowdfunding.php');
	}
	...
	public function auditAction() {
		if($this->_http->isPost()) {
			...
			if($result) {
				...
				$content = "一元众筹：{$getData['title']}　众筹ID：{$getData['tid']} " . ($getData['audit_type'] == 1?'<span style="color:green">审核通过</span>':'<span style="color:red">审核不通过</span>');
				Custom_Log::log($this->_userInfo['id'], $content, $this->pmodule, $this->cmodule, 'audit', 'ticket', $getData['tid']);
				Custom_Common::showMsg($getData['audit_type'] == 1?'审核通过':'审核不通过', '', array($backUrl => '返回一元众筹'));
			} else {
		...
		$this->_tpl->assign('page', $page);
		$this->_tpl->assign('row', $row);
		$this->_tpl->display('admin/ticket_audit_crowdfunding.php');
	}
	...
	public function lotteryAction() {
		...
		if(empty($row['ticket_uuid'])) {
			Custom_Common::showMsg('<span style="color:red">还没审核呢！</span>', 'back');
		}
		
		if($this->_http->isPost()) {
			$getData = $this->_http->getPost();
			
			if($row['lottery_time'] && REQUEST_TIME < $row['lottery_time']) {
				Custom_Common::showMsg('<span style="color:red">还没到抽奖时间呢！</span>', 'back');
			}
			//参加人数+阀值
			$ticketObj = Custom_AuthTicket::getTicketDetailByTicketUuid($row['ticket_uuid']);
			$threshold = $ticketObj->data->ProductDisplaySale;
			
			//取整((上证指数*深证指数)%(参加人数+阀值))+1
			$awardNumber = floor( ($getData['shenzhen_stock'] * $getData['shanghai_stock']) % $threshold) + 1;
			//远程确认中奖
			$resultArr = Custom_AuthTicket::lotteryWinning($row['ticket_uuid'], $awardNumber);
			if($resultArr['Code'] == 1) {
				$this->_model->updateLotteryStatus($resultArr['Result'], $tid);
				Custom_Common::showMsg(
					'恭喜，抽奖成功！',
					'',
					array(
						'lottery/tid:'. $getData['tid'] . '/page:' . $getData['page']   => '查看抽奖结果',
						'list/page:' . $getData['page'] => '返回一元众筹'
					)
				);	
			}		
		...
		$this->_tpl->assign('page', $page);
		$this->_tpl->assign('row', $row);
		$this->_tpl->display('admin/lottery_crowdfunding.php');
	}
	
	public function ajaxColAction() {
		$getData = $this->_http->getPost();
		Model_Admin_Ticket::getInstance()->ajax_module_edit($getData);
	}
	...
	public function recommendAction() {
		...
		if ($this->_http->isPost()) {
			...
			if($checkRepeat){
				Custom_Common::showMsg(
				'<span style="color:red">当前一元众酬商品在此推荐位重复，请重新选择推荐位</span>',
			...
			if($result){
				Custom_Log::log($this->_userInfo['id'], "新增了一元众酬名为  <b>{$ticketRow['ticket_title']}</b> 的推荐", $this->pmodule, $this->cmodule, 'recommend', 'ticket', $id);
				Custom_Common::showMsg(
				'推荐成功',
				'',
				array(
		...
		$this->_tpl->display('admin/recommend_crowdfunding_position.php');
	}
}