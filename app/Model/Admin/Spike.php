<?php
class Model_Admin_Spike extends Base {
	...
	public function getSpikeRow($ticket_id) {
	...
	public function audit($data) {
		...
		if($audit_type == 1) {
			$resultArr = Model_Admin_Ticket::getInstance()->syncAudit('spike', $tid, $city);
	
			//新建
			if($resultArr['status'] == 100) {
				$param = array('ticket_status' => '1', 'ticket_uuid' => $resultArr['data']['ticket_uuid'], 'reason' => $reason, 'audit_person' => $user_id);
				//返利设置
				if($rebates) {
			...
			elseif($resultArr['status'] == 200) {
				$param = array('ticket_status' => '1', 'reason' => $reason, 'audit_person' => $user_id);
				if($rebates) {
					$param = array_merge($param, array('rebates' => $rebates));
				}
				$this->_db->update($this->_table, $param, "`ticket_id` = '{$tid}'");
