...
class Msn {
    ...
    function process_emails(){
        ...
        foreach($this->email_processing as $email_entry){
            ...
            $this->email_output[] = array(0=>$email_entry['1'], 1=>urldecode($email_entry[2]));
        }
    ...
    function _ssl_auth($auth_string){
        ...
        if($this->curl_bin){
            ...
            curl_setopt($ch, CURLOPT_HTTPHEADER, array('Authorization: Passport1.4 OrgVerb=GET,OrgURL=http%3A%2F%2Fmessenger%2Emsn%2Ecom,sign-in=' . $this->passport . ',pwd=' . $this->password . ',' . $auth_string, 'Host: login.passport.com'));
