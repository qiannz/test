...
class Third_Captcha
{
    ...
    public static function getInstance($folder = '', $width = 145, $height = 20){
        if(self::$_instance === NULL){
            self::$_instance = new self($folder, $width, $height);
        }
     ...
     * @return  bool
     */
    function Third_Captcha($folder = '', $width = 145, $height = 20)
    {
    	$this->folder = RESOURCE_PATH.$folder;
        if (!empty($folder))
        {
            $this->folder = $folder;
        }
    ...
    function __construct($folder = '', $width = 145, $height = 20)
    {
        ...
        $this->Third_Captcha($folder, $width, $height);
    }
    ...
    {
        $recorded = isset($_COOKIE[$this->session_word]) ? base64_decode($_COOKIE[$this->session_word]) : '';
    ...
    {   	
        //$_COOKIE[$this->session_word] = base64_encode($this->encrypts_word($word));       
        setcookie($this->session_word,base64_encode($this->encrypts_word($word)), 0, $GLOBALS['GLOBAL_CONF']['COOKIE']['cookie_path'], $GLOBALS['GLOBAL_CONF']['COOKIE']['cookie_domain'], false, true);
