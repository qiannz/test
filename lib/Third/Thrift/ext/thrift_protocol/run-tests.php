...
$exts_to_test = array();
$ini_overwrites = array('output_handler=', 'open_basedir=', 'safe_mode=0', 'disable_functions=', 'output_buffering=Off', 'error_reporting=' . (E_ALL | E_STRICT), 'display_errors=1', 'display_startup_errors=1', 'log_errors=0', 'html_errors=0', 'track_errors=1', 'report_memleaks=1', 'report_zend_debug=0', 'docref_root=', 'docref_ext=.html', 'error_prepend_string=', 'error_append_string=', 'auto_prepend_file=', 'auto_append_file=', 'magic_quotes_runtime=0', 'ignore_repeated_errors=0', 'precision=14', 'unicode.runtime_encoding=ISO-8859-1', 'unicode.script_encoding=UTF-8', 'unicode.output_encoding=UTF-8', 'unicode.from_error_mode=U_INVALID_SUBSTITUTE');
...
More .INIs  : " , (function_exists(\'php_ini_scanned_files\') ? str_replace("\n","", php_ini_scanned_files()) : "** not determined **"); ?>';
    ...
    $info_params_ex = array('session'=>array('session.auto_start=0'), 'tidy'=>array('tidy.clean_output=0'), 'zlib'=>array('zlib.output_compression=Off'), 'xdebug'=>array('xdebug.default_enable=0'), 'mbstring'=>array('mbstring.func_overload=0'));
...
function save_text($filename, $text, $filename_copy = null){
    ...
    if($filename_copy && $filename_copy != $filename){
        if(file_put_contents($filename_copy, (string) $text, FILE_BINARY) === false){
...
function system_with_timeout($commandline, $env = null, $stdin = null){
    ...
    foreach((array) $env as $key=>$value){
        $bin_env[(string) $key] = (string) $value;
    }
    
    $proc = proc_open($commandline, array(0=>array('pipe', 'r'), 1=>array('pipe', 'w'), 2=>array('pipe', 'w')), $pipes, $cwd, $bin_env, array('suppress_errors'=>true, 'binary_pipes'=>true));
    ...
    while(true){
        ...
        if($n === false){
            ...
            $line = (string) fread($pipes[1], 8192);
            if(strlen($line) == 0){
                /* EOF */
                break;
            }
...
function binary_section($section){
    return PHP_MAJOR_VERSION < 6 || ($section == 'FILE' || $section == 'FILEEOF' || $section == 'EXPECT' || $section == 'EXPECTF' || $section == 'EXPECTREGEX' || $section == 'EXPECTHEADERS' || $section == 'SKIPIF' || $section == 'CLEAN');
}
...
";
    ...
    if(isset($section_text['EXPECTF']) || isset($section_text['EXPECTREGEX'])){
        ...
        if(isset($section_text['EXPECTF'])){
            ...
            $wanted_re = str_replace(array(b'%binary_string_optional%'), version_compare(PHP_VERSION, '6.0.0-dev') == -1 ? b'string' : b'binary string', $wanted_re);
            $wanted_re = str_replace(array(b'%unicode_string_optional%'), version_compare(PHP_VERSION, '6.0.0-dev') == -1 ? b'string' : b'Unicode string', $wanted_re);
            $wanted_re = str_replace(array(b'%unicode\|string%', b'%string\|unicode%'), version_compare(PHP_VERSION, '6.0.0-dev') == -1 ? b'string' : b'unicode', $wanted_re);
            $wanted_re = str_replace(array(b'%u\|b%', b'%b\|u%'), version_compare(PHP_VERSION, '6.0.0-dev') == -1 ? b'' : b'u', $wanted_re);
            // Stick to basics
...
*/
    ...
    if($failed_headers){
        $passed = false;
        $wanted = (string) $wanted_headers . b"\n--HEADERS--\n" . (string) $wanted;
        $output = (string) $output_headers . b"\n--HEADERS--\n" . (string) $output;
...
function comp_line($l1, $l2, $is_reg){
    if($is_reg){
        return preg_match(b'/^' . (string) $l1 . b'$/s', (string) $l2);
    }else{
        return !strcmp((string) $l1, (string) $l2);
    }
}
...
function generate_array_diff($ar1, $ar2, $is_reg, $w){
    ...
    while($idx1 < $cnt1 && $idx2 < $cnt2){
        ...
        if(comp_line($ar1[$idx1], $ar2[$idx2], $is_reg)){
            ...
            if($c1 > $c2){
                $old1[$idx1] = (string) sprintf("%03d- ", $idx1 + 1) . $w[$idx1++];
                $last = 1;
            }else if($c2 > 0){
                $old2[$idx2] = (string) sprintf("%03d+ ", $idx2 + 1) . $ar2[$idx2++];
                $last = 2;
            }else{
                $old1[$idx1] = (string) sprintf("%03d- ", $idx1 + 1) . $w[$idx1++];
                $old2[$idx2] = (string) sprintf("%03d+ ", $idx2 + 1) . $ar2[$idx2++];
    ...
    while($idx1 < $cnt1){
        $diff[] = (string) sprintf("%03d- ", $idx1 + 1) . $w[$idx1++];
    }
    
    while($idx2 < $cnt2){
        $diff[] = (string) sprintf("%03d+ ", $idx2 + 1) . $ar2[$idx2++];
    }
