...
class PHPRPC_Server {
    ...
    function errorHandler($errno, $errstr, $errfile, $errline){
        ...
        if(($errno == E_ERROR) or ($errno == E_CORE_ERROR) or ($errno == E_COMPILE_ERROR) or ($errno == E_USER_ERROR)){
            $this->errno = $errno;
            $this->errstr = $errstr;
            $this->sendError();
        }else{
            if(($errno == E_NOTICE) or ($errno == E_USER_NOTICE)){
                ...
                if(($this->errno == 0) or ($this->errno == E_NOTICE) or ($this->errno == E_USER_NOTICE)){
                    $this->errno = $errno;
                    $this->errstr = $errstr;
                }
            }
    ...
    function callFunction(){
        $this->initKey();
        $functionName = strtolower($this->getRequest('phprpc_func'));
        if(array_key_exists($functionName, $this->functions)){
            $function = $this->functions[$functionName];
            $arguments = $this->getArguments();
            
            // 防止SQL注入 处理传入参数 -- by lujun 2011.09.19
            $arguments = saddslashes($arguments);
            $callFunctionResult = $this->call($function, $arguments);
            
            // #debug log 写调试日志 -- by silver 2011.11.16
            Api_Abstract::log($functionName, $arguments);
            
            $result = $this->encodeString($this->encryptString(serialize_fix($callFunctionResult), 2));
            $output = ob_get_clean();
            ...
            $this->errno = E_ERROR;
            $this->errstr = "Can't find this function $functionName().";
            $output = ob_get_clean();
        }
