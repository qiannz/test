...
class _PHPRPC_Client {
    ...
    function invoke($funcname, &$args, $byRef = false){
        ...
        // PHP_RPC 调试模式中的错误捕获 -- silver 2011.08.03
        if(isset($GLOBALS['RPC_ModelConf']['Debug']) && $GLOBALS['RPC_ModelConf']['Debug']){
            if($phprpc_errno && $phprpc_errstr){ #debug silver
                echo 'Errno: ' . $phprpc_errno . '<br />ErrStr: ' . $phprpc_errstr;
            }
        }
        ...
        if(array_key_exists('phprpc_result', $result)){
            ...
            // PHP_RPC 调试模式中的错误捕获 -- silver 2011.08.03
            if(isset($GLOBALS['RPC_ModelConf']['Debug']) && $GLOBALS['RPC_ModelConf']['Debug']){
                if(!$this->_warning->Number && !$this->_warning->Message){
                    $this->_warning = new PHPRPC_Error(-9999, $result); // 一般都是服务端脚本语法错误
                }
            }
    ...
    function _post($request_body){
        ...
        while(!feof($this->_socket)){
            ...
            if($response_header instanceof PHPRPC_Error){
                $this->_disconnect();
    ...
    function _parseHeader($header){
        ...
        if(isset($header['set-cookie'])){
            foreach($header['set-cookie'] as $cookie){
                foreach(preg_split('/[;,]\s?/', $cookie) as $c){
                    $tmpSplit = explode('=', $c, 2);
                    $name = isset($tmpSplit[0]) ? $tmpSplit[0] : '';
                    $value = isset($tmpSplit[1]) ? $tmpSplit[1] : '';
        ...
        return array('transfer_encoding'=>$transfer_encoding, 'content_encoding'=>$content_encoding, 'content_length'=>$content_length, 'connection'=>$connection);
    }
