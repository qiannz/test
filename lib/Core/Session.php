...
class Core_Session
{
    ...
    function cls_session(&$db, $session_table, $session_data_table, $session_name = '', $session_id = '')
    {
        $GLOBALS['_SESSION'] = array();

        if (!empty($GLOBALS['GLOBAL_CONF']['COOKIE']['cookie_path']))
        {
            $this->session_cookie_path = $GLOBALS['GLOBAL_CONF']['COOKIE']['cookie_path'];
        }
        else
        {
        ...
        if (!empty($GLOBALS['GLOBAL_CONF']['COOKIE']['cookie_domain']))
        {
            $this->session_cookie_domain = $GLOBALS['GLOBAL_CONF']['COOKIE']['cookie_domain'];
        }
        else
        {
        ...
        if (!empty($GLOBALS['GLOBAL_CONF']['COOKIE']['cookie_secure']))
        {
            $this->session_cookie_secure = $GLOBALS['GLOBAL_CONF']['COOKIE']['cookie_secure'];
        }
        else
        {
        ...
        $this->db  = &$db;
        $this->_ip = Custom_Client::getUserIp();
    ...
    function gen_session_key($session_id)
    {
        ...
        return sprintf('%08x', crc32(ROOT_PATH . $session_id));
    }

    function insert_session()
    {
    	$sql = "select count(*) from ".$this->session_table." where ip = '".$this->_ip."' and expiry > " . ($this->_time - $this->max_life_time) ;
    	$num = $this->db->fetchOne($sql);
    	if($num >= 100){
    		$this->db->query("DELETE FROM " . $this->session_table . " WHERE ip = '".$this->_ip."'");
    	}
    ...
    {
        $session = $this->db->fetchRow('SELECT userid, adminid, user_name, data, expiry FROM ' . $this->session_table . " WHERE sesskey = '" . $this->session_id . "'");
        ...
        else
        {
            ...
            {
                $session_data = $this->db->fetchRow('SELECT data, expiry FROM ' . $this->session_data_table . " WHERE sesskey = '" . $this->session_id . "'");
    ...
    function update_session()
    {
        ...
        return $this->db->query('UPDATE ' . $this->session_table . " SET expiry = '" . $this->_time . "', ip = '" . $this->_ip . "', userid = '" . $userid . "', adminid = '" . $adminid . "', user_name='" . $user_name . "', data = '$data' WHERE sesskey = '" . $this->session_id . "' LIMIT 1");
    ...
    {
        return $this->db->fetchOne('SELECT count(*) FROM ' . $this->session_table);
    }
}